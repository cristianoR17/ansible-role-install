...
#Data 11-10-2016 Adcionada funcao de backup do SIP WEB
# Obs.:
...
nomeCliente=bkp_prefeiturasp
dirShare=//brentfs202/backupcomunix$
dirDestino=/mnt/share
cpDestino=/mnt/share/backup_comunix/Mooca
usuario=comunix
senha=12345678
...
asteriskCfgBkp(){
    if [ -x /usr/sbin/asterisk ] ; then 
        ...
        tar zcf $arquivoTar asterisk 2> /dev/null
        #Fazendo backup dos arquivos no /var/www/
        cd /var/
        arquivoTar="$bkpDiretorio"/"$data"_var_www_"$nomeServidor".tar.gz
        tar zcf $arquivoTar www/ 2> /dev/null	
	cd /usr/lib/
...
certificadoBkp(){
    if [ -d /home/certificado ] ; then

        tipo=OMNI
        geraDiretorio
        #Fazendo backup das configuracoes do certificado
        cd /home/
        arquivoTar="$bkpDiretorio"/"$data"_home_certificado_"$nomeServidor".tar.gz
        tar zcf $arquivoTar certificado 2> /dev/null
        cd $diretorioTemp
        arquivoBkp=bkp_"$nomeServidor"_"$tipo"_"$data".tar.gz
        tar zcf $arquivoBkp bkp_"$nomeServidor" 2> /dev/null && rm -r $bkpDiretorio
        mv $arquivoBkp $destinoBkp
...
#Fazendo o backup da parte web do SIP
certificadoBkp
qtdBkp=5
checkQtdBackup
#enviaFtp
